...
/*
 * Processor 0 (P0) is the first processor in the system.
 * It is first processor out of the four processors in the system.
 * 
 * Responsibilities:
...
// -- our mutex and interrupt variables --
void *context;
alt_mutex_dev* mutex;
int received = 0;
int valueFromP1 = 0;
...
/*
 * interrupt function triggered by p1
 * triggered by a signal from p1. 
*/
void KEY_ISR(void *isr_context, alt_u32 id){
	...
	IOWR(P0_IN_BASE,3,0x1);
}

/*
 * function to send message to P1
 * does so via the shared buffer
*/
...
//	altera_avalon_mutex_unlock(mutex);
	...
	//pulsing the output
	IOWR(P0_OUT_BASE,0,1);
	IOWR(P0_OUT_BASE,0,0);
...
/*
 * function to convolve an image with a 2D kernel
 * perfroms first a horizontal pass and then a vertical pass
*/
...
/*
 * function used to calculate and then display the run time of system. 
 * fps is displayed on the 7-segment display
 * 
...
/*
 * main function for P0
 * this is the main function that runs on P0
 * it handles the image processing tasks and display tasks
 * it also handles the interrupts from P1
*/
int main()
{ 
	...
	IOWR(P0_IN_BASE,3,0x1);
	IOWR(P0_IN_BASE,2,0x1);
	alt_irq_register(P0_IN_IRQ, &context, KEY_ISR);
	int counter  = 0;
...
//	  				printf("in edge detect \n");
	  				...
	  				img4 = CONV_RESULT_BASE_2;
//	  				printf("img4 assigned as edge detect \n");
